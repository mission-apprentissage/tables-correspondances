...
class BcnController {
  ...
  async getDataFromCfd(providedCfd) {
    ...
    const libCourtUpdated = await this.findLibCourt(cfdUpdated.value, lookupBase);
    const niveauFormationUpdated = await this.findNiveauFormationDiplome(cfdUpdated.value, lookupBase);
